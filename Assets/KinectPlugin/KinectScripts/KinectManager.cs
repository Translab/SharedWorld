...
//				break;
	...
	public void ResetAvatarControllers()
	{
		if(Player1Avatars.Count == 0 && Player2Avatars.Count == 0)
		{
			...
			//hack here here//
			//////////////////
			if (avatars != null && avatars.Length >= 2){
			Player1Avatars.Add(avatars[0].gameObject);
			Player2Avatars.Add(avatars[1].gameObject);
			}
	...
	void Awake()
	{
		...
		if(ComputeUserMap)
		{
			...
			Debug.Log("You click me 1::" + usersMapSize);
							...
							gestureData.progress >= 0.5f)
						{
							if(GetGestureProgress(gestureData.userId, KinectGestures.Gestures.Click) < 0.3f)
							{
								...
								if(ControlMouseCursor)
								{
									...
									MouseControlKinect.MouseMove(vCursorPos, CalibrationText);
								}
							}
						}
		...
		if(KinectInitialized)
		{
			...
			else if(ComputeColorMap && (/**(allUsers.Count == 0) ||*/ DisplayColorMap))
			{
				if(usersClrRect.width == 0 || usersClrTex.height == 0)
				{
					...
					float displayWidth = cameraRect.width * displayMapsWidthPercent*5;
					float displayHeight = cameraRect.width * displayMapsHeightPercent*5;
